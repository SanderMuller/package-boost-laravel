...
**Important**: Only edit files in `.ai/`. The `.claude/skills/` and `.github/skills/` directories are auto-generated by `vendor/bin/boost sync` and will be overwritten.
...
**CRITICAL: Always run `boost sync` before committing any changes to `.ai/` files.**
...
**All these files must be committed together** — they are kept in sync by `boost sync`.
...
- [ ] **Run `vendor/bin/boost sync`** (CRITICAL!)
